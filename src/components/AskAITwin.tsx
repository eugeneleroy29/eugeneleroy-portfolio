...
import { Bot, Check, CornerDownLeft, Loader2, Send, Sparkles, User } from 'lucide-react';
...
export function AskAITwin() {
  ...
  return (
    <section id="ai-twin" className="py-12 border-b border-zinc-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Chat Console */}
        <div className="rounded-2xl bg-zinc-900/70 border border-zinc-800 p-4 sm:p-6 shadow-2xl flex flex-col gap-4">
          {/* Sample Prompts */}
          <div className="flex flex-wrap items-center gap-2">
            ...
            {SAMPLE_QUESTIONS.map((q, idx) => (
              <button
                ...
                className="px-2.5 py-1 rounded-lg bg-zinc-800/80 hover:bg-zinc-800 text-zinc-300 border border-zinc-700/60 text-[11px] transition"
              >
                {q}
              </button>
            ))}
          ...
          <div className="min-h-[220px] max-h-[360px] overflow-y-auto space-y-3.5 pr-2">
            {messages.map((m, idx) => (
              ...
              >
                {m.role === 'assistant' && (
                  <div className="h-7 w-7 rounded-lg bg-indigo-600/20 border border-indigo-500/40 flex items-center justify-center shrink-0">
          ...
          >
            <input
              ...
              className="flex-1 bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-xs text-zinc-200 placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
            />
            <button
              type="submit"
              disabled={isLoading || !input.trim()}
              className="px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs font-semibold flex items-center gap-1.5 transition"
            >
